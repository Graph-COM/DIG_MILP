...
def main():
    ...
    parser.add_argument('--n_nodes', dest = 'n_nodes', type = int, default = 1000, help = 'number of rows')
    parser.add_argument('--n_items', dest = 'n_items', type = int, default = [40, 60, 80, 100, 120, 140, 160], help = 'the number of instances')
    parser.add_argument('--n_bids', dest = 'n_bids', type = float, default = [200, 300, 400, 500, 600, 700, 800], help = 'density')
    parser.add_argument('--seed', dest = 'seed', type = int, default = 123, help = 'random seed')
    parser.add_argument('--max_coeff', dest = 'max_coeff', type = int, default = 10, help = 'maximum coefficient of objective')
    args = parser.parse_args()
    
    for n_item, n_bid in zip(args.n_items, args.n_bids):
        for instance_idx in tqdm(range(args.num_instance)):
            if not os.path.exists(args.primal_folder+'nitem'+str(int(n_item))+'/'):
                os.mkdir(args.primal_folder+'nitem'+str(int(n_item)))
            lp_file = args.primal_folder+'nitem'+str(int(n_item))+'/'+str(instance_idx)+'.lp'
            random_seed = random.randint(0,99999)
            rng = np.random.RandomState(random_seed)
            generate_cauctions(rng, lp_file, n_item, n_bid)
