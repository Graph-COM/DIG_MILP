...
from _2_generate_train_data import CauctionsDownstream, BipartiteData
from _8_generate_val_data import CauctionsDownstreamVal, BipartiteData
from _5_generate_test_graph import CauctionsDownstreamTest, BipartiteData
...
def main():
    ...
    parser.add_argument('--generate_primal', dest = 'generate_primal', type = str, default = '../data/generate_primal/020_a150lr1e3/', help = 'which folder to get the generated primal instances')
    ...
    parser.add_argument('--save_folder', dest = 'save_folder', type = str, default = './dataset/020_a150lr1e3/', help = 'the folder to save the graph data')
    ...
    parser.add_argument('--generate_primal_solution', dest = 'generate_primal_solution', type = str, default = './dataset/generate_primal_solution/020_a150lr1e3/', help = 'the folder to get the primal solution')
    parser.add_argument('--normalize_statistics', dest = 'normalize_statistics', type = str, default = './dataset/020_a150lr1e3/', help = 'the folder to get the normalize data')
    # Model parameters
    # Predictor
    ...
    # Init wandb
    if args.wandb:
        wandb.init(project="cauctions_downstream")
    ...
    dataset = CauctionsDownstream(args.primal, args.generate_primal, args.num_instance, args.ratio, save_folder, args.primal_solution, args.generate_primal_solution, normalize_statistics)
    dataloader = DataLoader(dataset, batch_size=args.batch, shuffle = True, follow_batch = ['x_s', 'x_t'])

    val_primal ='./testset/'+'val/'
    val_save_folder = './testset/'+'val/'
    valset = CauctionsDownstreamVal(val_primal,args.num_instance, val_save_folder)
    ...
    # testing
    predictor.eval()
    n_items = [40, 60, 80, 100, 120, 140, 160]
    n_bids = [200, 300, 400, 500, 600, 700, 800]
    for n_item, n_bid in zip(n_items, n_bids):
        lp_file = './testset/'+'nitem'+str(int(n_item))+'/'
        save_folder = './testset/'+'nitem'+str(int(n_item))+'/'
        testset = CauctionsDownstreamTest(lp_file, args.num_instance, save_folder)
