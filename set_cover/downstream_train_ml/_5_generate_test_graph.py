...
class SetCoverDownstreamTest(InMemoryDataset):
    def __init__(self, primal, num_instance, density, save_folder):
        self.save_folder = Path(save_folder)
        self.density = density
        self.num_instance = num_instance
        self.primal = primal
        super(SetCoverDownstreamTest, self).__init__(root = self.save_folder)
...
def main():
    ...
    parser.add_argument('--density', dest = 'density', type = float, default = 0.10, help = 'density')
    ...
    primal = args.primal+'density'+str(int(args.density*100))+'/'
    save_folder = args.save_folder+'density'+str(int(args.density*100))+'/'
    dataset = SetCoverDownstreamTest(primal, args.num_instance, args.density, save_folder)
