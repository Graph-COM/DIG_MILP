...
def main():
    ...
    parser.add_argument('--density', dest = 'density', type = float, default = 0.40, help = 'density')
    ...
    for instance_idx in tqdm(range(args.num_instance)):
        if not os.path.exists(args.primal_folder+'density'+str(int(args.density*100))+'/'):
            os.mkdir(args.primal_folder+'density'+str(int(args.density*100)))
        lp_file = args.primal_folder+'density'+str(int(args.density*100))+'/'+str(instance_idx)+'.lp'
        random_seed = random.randint(0,99999)
        rng = np.random.RandomState(random_seed)
        generate_setcover(nrows = args.nrows, ncols = args.ncols, density = args.density, filename = lp_file, rng = rng, max_coef = args.max_coeff)
