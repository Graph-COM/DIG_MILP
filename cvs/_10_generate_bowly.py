...
from _bowly_lhs_generator import generate_lhs
...
def main():
    ...
    for batch_data in tqdm(dataloader):
        ...
        predict_degree = torch.rand_like(predict_degree).to(device)
        predict_logits = torch.rand_like(predict_logits).to(device)
        predict_weights = torch.rand_like(predict_weights).to(device)
        predict_num_x = torch.rand_like(predict_num_x).to(device)
        predict_x = torch.rand_like(predict_x).to(device)
        predict_ym = torch.rand_like(predict_ym).to(device)
        predict_yn = torch.rand_like(predict_yn).to(device)
        predict_s = torch.rand_like(predict_s).to(device)
        predict_rm = torch.rand_like(predict_rm).to(device)
        predict_rn = torch.rand_like(predict_rn).to(device)
        ...
        # here are bowly generator
        #variable_list = [200,300,400]
        #variables = random.choice(variable_list)
        #constraints = int(np.random.normal(100, np.sqrt(30)))
        variables = batch_data.x_t.shape[0]
        constraints = batch_data.x_s.shape[0]
        density = 0.0013
        ...
        coeff_loc= 0.2739
        coeff_scale= 0.961
        matrix_a = generate_lhs(variables, constraints, density, pv, pc,
                            coeff_loc, coeff_scale, random_state)
        batch_data.edge_attr = torch.tensor(matrix_a.data)
        batch_data.edge_attr = batch_data.edge_attr.round(decimals = 1)
        batch_data.edge_index = torch.cat((torch.tensor(matrix_a.row).reshape(1,-1),torch.tensor(matrix_a.col).reshape(1,-1)),0)
