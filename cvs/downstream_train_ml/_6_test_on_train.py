...
def main():
    ...
    parser.add_argument('--gpu', dest = 'gpu', type = int, default = 6, help = 'the index of GPU')
    ...
    parser.add_argument('--train_folder', type = str, dest = 'train_folder', default = './train_files/050/', help = 'folder to save the trained models')
    ...
    parser.add_argument('--generate_primal', dest = 'generate_primal', type = str, default = '../data/generate_primal/050_random/', help = 'which folder to get the generated primal instances')
    ...
    parser.add_argument('--generate_primal_solution', dest = 'generate_primal_solution', type = str, default = './dataset/generate_primal_solution/050_random/', help = 'the folder to get the primal solution')
    parser.add_argument('--normalize_statistics', dest = 'normalize_statistics', type = str, default = './dataset/050_random/', help = 'the folder to get the normalize data')
    parser.add_argument('--save_folder', dest = 'save_folder', type = str, default = './dataset/050_random/', help = 'the folder to save the graph data')
    # Model parameters
    # Predictor
    ...
    state_dict = torch.load(train_folder+'predictor.pth', map_location = device)
    predictor.to(device)
    ...
    normalize_statistics = args.normalize_statistics + 'ratio' + str("{:.2f}".format(args.ratio)).replace('.', '') + '/'
    predictor.eval()
    test_list = ['cvs08r139-94','cvs16r70-62','cvs16r89-60']
    primal_solution_path = '../data/primal_solution/'
    save_folder = './testset/val/'
    
    testset = CVSDownstreamVal(args.primal, primal_solution_path, test_list, save_folder)
    ...
    for test_data in testloader:
        ...
        print('y label:'+str(unnormalized_y_label))
        print('y predict:'+str(predict_y_test))
        print('test msre:'+str(rmse))
