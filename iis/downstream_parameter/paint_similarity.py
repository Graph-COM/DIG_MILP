...
def main():
    ...
    parser.add_argument('--save_folder', dest = 'save_folder', type = str, default = '../downstream', help = 'which folder to get the lp instances')
    ...
    for trial_id in range(0,3):
        ...
        for seed in range(45):
            save_folder = './time_log'+str(trial_id)+'/'  + str(seed) + '/' 
            save_folder2 = './time_log'+str(trial_id)+'/' + str(seed) + '/' 
    ...
    '''for i, (x_val, y_val) in enumerate(zip(primal_list_mean, generate_list_mean1)):
        plt.annotate(str(i), (x_val, y_val), textcoords="offset points", xytext=(0, 0), ha="center")'''

    plt.xlabel('original')
    plt.ylabel('random (ratio = 0.1)')
    plt.yticks([])
    plt.tight_layout()
    ...
    '''for i, (x_val, y_val) in enumerate(zip(primal_list_mean, generate_list_mean2)):
        plt.annotate(str(i), (x_val, y_val), textcoords="offset points", xytext=(0, 0), ha="center")'''
    plt.xlabel('original')
    plt.ylabel('random (ratio = 0.2)')
    plt.yticks([])
    plt.tight_layout()
    ...
    '''for i, (x_val, y_val) in enumerate(zip(primal_list_mean, generate_list_mean3)):
        plt.annotate(str(i), (x_val, y_val), textcoords="offset points", xytext=(0, 0), ha="center")'''
    plt.xlabel('original')
    plt.ylabel('random (ratio = 0.3)')
    plt.yticks([])
    plt.tight_layout()
